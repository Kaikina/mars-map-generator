...
describe.only('User api', function() {
    ...
    describe('POST user api', function() {
        xit("La requete envoie tous les données d'un user, l'user est crée et on reçoit un 200", async () => {
        ...
        xit("La requete envoie n'evoie pas tous les données d'un user, on reçoit un 400", async () => {
            await server.post('/api/v1/users')
                .send({
                    username: 'Tulkass'
                })
        ...
        xit("La requete envoie un utilisateur qui existe déjà, on reçoit un 409", async () => {
            await server.post('/api/v1/users')
                .send({
                    username: 'Kaikina',
                    fullName: 'Girou Tom',
    ...
    describe('GET /api/v1/users/:username', function() {
        xit("Given username doesn't exist, then send 404", async () => {
            await server.get('/api/v1/users/je-n-existe-pas')
                .expect(404);
        });

        xit("Given username exists then send 200 and user", async () => {
    ...
    describe('PUT /api/v1/users/:username', function() {
        ...
        xit("Given username exists then send 200 and user updated", async () => {
        ...
        xit("Given data is incorrect then send 400", async () => {
