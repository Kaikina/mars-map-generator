...
describe('Flight api', function() {
	...
	describe('POST /api/v1/flights', function() {
		xit('should create a flight', async () => {
		...
		xit('should return an 400 error if given body has not all the mandatory data', async () => {
	...
	describe.only('GET /api/v1/flights/:reference', function() {
		xit("La reference donnée n'existe pas alors j'ai un 404", async () => {
			await server.get('/api/v1/flights/je-n-existe-pas')
				.expect(404);
		});

		xit("La reference donnée existe donc j'ai un 200 avec un vol", async () => {
